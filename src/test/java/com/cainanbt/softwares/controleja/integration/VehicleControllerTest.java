...
public class VehicleControllerTest extends BaseTest {
    ...
    void setup() {
        ...
        // 3. Cria Conta e Categoria para usar nas transações
    ...
    void shouldCreateVehicle() {
        ...
        dto.setCurrentOdometer(new BigDecimal("100000.00")); // 100 mil km
    ...
    @Test
    @DisplayName("INTEGRAÇÃO: Abastecimento deve atualizar Hodômetro do carro")
    void shouldUpdateOdometerOnRefuel() {
        // 1. Cadastra Carro com 10.000 KM
        VehicleDTO vDto = new VehicleDTO();
        vDto.setName("Civic Teste");
        vDto.setBrand("Honda");
        vDto.setModel("Civic");
        vDto.setYear(2020);
        ...
        // 2. Lança Transação: Abasteceu 500 km depois (10.500 km)
        ...
        tDto.setCurrentOdometer(new BigDecimal("10500.00")); // KM NOVO
        tDto.setLiters(50.0); // 50 Litros
        ...
        // 3. Verifica se o carro atualizou sozinho para 10.500
        given()
                .header("Authorization", "Bearer " + token)
                .when()
                .get("/vehicles")
    ...
    void shouldNotUpdateOdometerIfLower() {
        ...
        tDto.setCategoryId(categoryId);
        tDto.setVehicleId(vehicleId);
        tDto.setCurrentOdometer(new BigDecimal("19000.00"));

        given().header("Authorization", "Bearer " + token).contentType(ContentType.JSON).body(tDto).post("/transactions").then().statusCode(200);
